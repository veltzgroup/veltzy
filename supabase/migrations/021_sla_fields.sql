-- Campos para controle de SLA e tempo de resposta
ALTER TABLE veltzy.leads
ADD COLUMN IF NOT EXISTS last_customer_message_at TIMESTAMPTZ,
ADD COLUMN IF NOT EXISTS sla_breached BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS first_response_at TIMESTAMPTZ;

CREATE INDEX IF NOT EXISTS idx_leads_last_customer_message
ON veltzy.leads(company_id, last_customer_message_at);

-- Atualizar RPC para incluir novos campos
CREATE OR REPLACE FUNCTION veltzy.get_conversation_list(p_company_id UUID)
RETURNS TABLE (
  id UUID,
  company_id UUID,
  name TEXT,
