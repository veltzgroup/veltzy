...
const StageRow = ({ stage }: { stage: PipelineStage }) => {
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border border-border/20 p-2">
      ...
      <input type="color" value={color} onChange={(e) => setColor(e.target.value)} onBlur={handleBlur} className="h-6 w-6 cursor-pointer rounded border-0" />
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-border/20">
          <input type="color" value={newColor} onChange={(e) => setNewColor(e.target.value)} className="h-7 w-7 cursor-pointer rounded border-0" />
