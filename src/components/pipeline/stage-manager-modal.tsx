...
import {
  usePipelineStages, useCreateStage, useUpdateStage, useDeleteStage, useReorderStages,
...
const SortableStageRow = ({
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-2 rounded-lg border border-border/50 p-2">
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={stages?.map((s) => s.id) ?? []} strategy={verticalListSortingStrategy}>
            <div className="space-y-2 max-h-[50vh] overflow-y-auto">
              {stages?.map((stage) => (
                <SortableStageRow key={stage.id} stage={stage} leadCount={leadCounts[stage.id] ?? 0} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
